...
MAX_PATIENTS = 254  # Maximale Anzahl Patienten
...
def parse_patient_folder(foldername):
    ...
    """
    parts = foldername.split('-')  # ["HN","CHUM","001"]
    if len(parts) < 3:
        return None, None
...
def find_nii_file_in_subdirs(folder):
    ...
    for root, dirs, files in os.walk(folder):
        # Skip Ordner, die "TomoTherapy" enthalten
...
def main():
    ...
    writer.writerow(["pid", "study_yr", "combination"])  # Kopfzeile

    # 1) Durchlaufe alle Ordner in SOURCE_DIR
    ...
    for folder in patient_folders:
        ...
        # 2) Finde .nii / .nii.gz (ignoriere "TomoTherapy")
        nii_path = find_nii_file_in_subdirs(folderpath)
        if nii_path is None:
            print(f"Keine NIfTI-Datei gefunden in: {folderpath} => überspringe")
            continue

        # 3) Bilde Zieldateiname => pid_HNCHUM_study_yr_001.nii.gz
        #    (Behalte ggf. .nii oder .nii.gz)
